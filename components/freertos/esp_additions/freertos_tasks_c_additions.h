...
_Static_assert( offsetof( StaticTask_t, pxDummy8 ) == offsetof( TCB_t, pxEndOfStack ) ); //changed here: commented this line
                                        ...
                                        TickType_t xOriginalDeadline )
    {
        ...
        #else /* CONFIG_FREERTOS_SMP */
        {
            ...
            if( pxNewTCB != NULL )
            {
                ...
                /* Set the deadlines in the TCB */
                TickType_t absoluteDeadline = xTaskGetTickCount() + xOriginalDeadline;
                pxNewTCB->xBaseDeadline = xOriginalDeadline;
                pxNewTCB->xCurrentDeadline = absoluteDeadline;
                pxNewTCB->xAbsoluteDeadline = absoluteDeadline;
                pxNewTCB->xStateListItem.xItemValue = absoluteDeadline; //changed here
                pxNewTCB->xEventListItem.xItemValue = absoluteDeadline;
                vListInitialiseItem( &( pxNewTCB->xWaitingListItem ) );
                listSET_LIST_ITEM_OWNER(&(pxNewTCB->xWaitingListItem), pxNewTCB);
                pxNewTCB->xWaitingListItem.xItemValue = absoluteDeadline;
                                                ...
                                                TickType_t xOriginalDeadline )
    {
        ...
        #else /* CONFIG_FREERTOS_SMP */
        {
            ...
            if( ( pxTaskBuffer != NULL ) && ( puxStackBuffer != NULL ) )
            {
                ...
                /* Set the deadlines in the TCB */
                pxNewTCB->xCurrentDeadline = xOriginalDeadline;
                pxNewTCB->xAbsoluteDeadline = xOriginalDeadline;
                pxNewTCB->xStateListItem.xItemValue = xOriginalDeadline; //changed here
                pxNewTCB->xEventListItem.xItemValue = xOriginalDeadline;
                vListInitialiseItem( &( pxNewTCB->xWaitingListItem ) );
                listSET_LIST_ITEM_OWNER(&(pxNewTCB->xWaitingListItem), pxNewTCB);
                pxNewTCB->xWaitingListItem.xItemValue = xOriginalDeadline;
            ...
            #if ( configUSE_MUTEXES == 1 )
            {
                ...
                if( uxNewPriority > pxTCB->uxBasePriority )
                {
                    ...
                    if( uxNewPriority > pxTCB->uxPriority )
                    {
                        ...
                        if( listIS_CONTAINED_WITHIN( &pxReadyTasksList, &( pxTCB->xStateListItem ) ) != pdFALSE )
                        {
